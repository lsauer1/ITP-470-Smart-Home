<?php
require 'config.php';
if (isset($_SESSION['email']) && isset($_GET['id']) && isset($_POST['name']) && isset($_POST['household_id'])) {
  ...
  $stmt->close();
  if (!($stmt = $mysqli->prepare("UPDATE devices SET name = ?, household_id = ? WHERE device_id = ?;"))) {
    echo '<div class="text-danger font-italic">('.$mysqli->errno.") " . $mysqli->error.'</div>';
  }
  $stmt->bind_param("sii",$_POST['name'], $_POST['household_id'], $_GET['id']);
  if (!$stmt->execute()) {
    echo '<div class="text-danger font-italic">'.$stmt->error.'</div>';
    exit();
  }

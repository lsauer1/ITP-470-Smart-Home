<?php
require 'config.php';
if (isset($_SESSION['email']) && isset($_GET['id']) && isset($_POST['name'])) {
  ...
  $stmt->close();
  if (!($stmt = $mysqli->prepare("UPDATE households SET name = ? WHERE id = ? AND owner_id = ?;"))) {
    echo '<div class="text-danger font-italic">('.$mysqli->errno.") " . $mysqli->error.'</div>';
  }
  $stmt->bind_param("sii", $_POST['name'], $_GET['id'], $user["user_id"]);
  if (!$stmt->execute()) {
    echo '<div class="text-danger font-italic">'.$stmt->error.'</div>';
    exit();
  } else{
    echo '<div class="text-success font-italic">Update Successful!</div>';

...
<body>
    ...
    <div class="body">
  <?php 
  if (isset($_GET['id']) && isset($_GET['feed'])) {
    $stmt = $mysqli->prepare("DELETE FROM devices WHERE device_id=?;");
    ...
    if (!$stmt->execute()) {
      ...
      sendCURL('https://io.adafruit.com/api/v2/'.$master_aiouser.'/feeds/'.$_GET['feed'], array(), array('X-AIO-KEY: '.$master_aiokey), "DELETE");
      echo '<div class="text-success">Device was successfully deleted.</div>';

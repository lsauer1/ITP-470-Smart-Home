...
if (isset($_SESSION['email'])) {
  ...
  foreach ($household_relations as $houseid) {
    $query = "SELECT *, routines.id AS routine_id, routines.name AS routine_name, households.name as household_name FROM routines JOIN households ON households.id = routines.household_id WHERE routines.household_id = ?";
    ...
    $result = $stmt->get_result();
    for ($i = 0; $i < $result->num_rows; $i++) {
      array_push($routines, $result->fetch_assoc());
    }
    $favorite = array_column($devices, 'favorite');

    array_multisort($routines, SORT_ASC, $favorite);
    $stmt->close();
  }  
} else {
...
<body>
    ...
    <div class="container-fluid"> 
      ...
      <div class="collapse navbar-collapse" id="navbar-toggler">
        <div class="navbar-nav mr-auto">
          ...
          <a class="nav-item nav-link text-white active" id="routine-link" href="routines.php">Routines</a>
        </div>
  ...
  <div class="body">
    <div class="row justify-content-center align-items-center runover-row mobile-show">
      <div class="col-12">
        <h3 class="text-center">Create Routine:</h3>
      </div>
    </div>
    <div class="row justify-content-center align-items-center runover-row mobile-show">
      <div class="col-12">
          ...
          <div class="row justify-content-center align-items-center mobile-show">
            <div class="col-12">
              <div class="input-group mb-3 flex-nowrap">
                ...
                <input required type="text" name="name" placeholder="Routine Name" class="form-control">
          ...
          <table class="table table-hover table-striped">
            ...
            <tbody>
              ...
              foreach ($routines as $routine) {
                echo '<tr>';
                if ($routine["favorite"]) {
                  echo '<td scope="row"><a href="favorite_routine.php?id='.$routine["routine_id"].'&favorite=0&return=1" class="text-white"><i class="fas fa-star"></i></a></td>';
                } else {
                  echo '<td scope="row"><a href="favorite_routine.php?id='.$routine["routine_id"].'&favorite=1&return=1" class="text-white"><i class="far fa-star"></i></a></td>';
                }
                echo '<td><a href="edit_routine.php?id='.$routine["routine_id"].'" class="btn btn-outline-light device-name">'.$routine["routine_name"]."</a></td>";
                echo "<td class='mobile-remove'><span class='nav-link text-white'>".$routine["household_name"]."</span></td>";
                echo '<td class="mobile-remove"><div class="row"><a class="btn btn-primary spacing-right" href="edit_routine.php?id='.$routine["routine_id"].'"><i class="fa fa-edit" aria-hidden="true"></i></a><a class="btn btn-danger" onclick="return confirm("Are you sure you want to delete this routine?")" href="delete_routine.php?id='.$routine["routine_id"].'"><i class="fa fa-trash" aria-hidden="true"></i></a></div></td>';
              }
              ?>
            </tbody>
          </table>
